...
//   // Acessing searchParams in client component
...
//                 type="submit"
//                 disabled={isLoading}
//                 className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed hover:cursor-pointer"
//               >
...
import { createUser } from "../_lib/database-actions";
...
export default function SignIn() {
  ...
  const query = searchParams.get("action");
  console.log(query);
  const isLogin = searchParams.get("action") === "signup" ? false : true;
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // After signup, redirect them to login
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          >
              ...
              <div>
                ...
                <div className="mt-1">
                  <input
                    ...
                    className="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-black"
            ...
            <div>
              ...
              >
                {isLoading ? <MiniSpinner /> : isLogin ? "Login" : "Sign Up"}
              </button>
            </div>
          </form>
